#!/bin/bash
#SBATCH --job-name=baseline_guided_track
...
#SBATCH --output=../logs/slurm-baseline-guided-track-%j.out
#SBATCH --error=../logs/slurm-baseline-guided-track-%j.err
...
# Run the pipeline
echo "Starting baseline-guided DNN training with track-matching features..."
echo "Time started: $(date)"

python scripts/train.py --config-file config/configs/experiment_baseline_guided_track.yaml

python scripts/evaluate.py --model-dir /pscratch/sd/l/liangyu/vertextiming/models/baseline_guided_dnn_with_tracks --load-data
