#!/bin/bash
#SBATCH --job-name=multi_transformer_jets_tracks
...
#SBATCH --output=../logs/slurm-multi-transformer-%j.out
#SBATCH --error=../logs/slurm-multi-transformer-%j.err
...
# Run the pipeline
echo "Starting Multi-input Transformer training with jets and tracks..."
echo "Time started: $(date)"

python scripts/train.py --config-file config/configs/experiment_transformer_with_jets_tracks.yaml

python scripts/evaluate.py --model-dir /pscratch/sd/l/liangyu/vertextiming/models/multi_input_transformer_with_jets_tracks --load-data
