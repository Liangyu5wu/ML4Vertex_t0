...
#SBATCH --output=../logs/slurm-hgtd-only-dnn-%j.out
#SBATCH --error=../logs/slurm-hgtd-only-dnn-%j.err
...
# Run the pipeline
echo "Starting HGTD-only DNN training using only HGTD tracks (no LAr data)..."
echo "Time started: $(date)"

python scripts/train.py --config-file config/configs/experiment_hgtd_only.yaml

python scripts/evaluate.py --model-dir /pscratch/sd/l/liangyu/vertextiming/models/hgtd_only_dnn --load-data
