#!/bin/bash
#SBATCH --job-name=hgtd_multi_dnn_jets_tracks
...
#SBATCH --output=../logs/slurm-hgtd-multi-dnn-%j.out
#SBATCH --error=../logs/slurm-hgtd-multi-dnn-%j.err
...
# Run the pipeline
echo "Starting HGTD Multi-input DNN training with LAr cells, jets, LAr tracks, and HGTD tracks..."
echo "Time started: $(date)"

python scripts/train.py --config-file config/configs/experiment_hgtd_dnn_with_jets_tracks.yaml

python scripts/evaluate.py --model-dir /pscratch/sd/l/liangyu/vertextiming/models/hgtd_multi_input_dnn_with_jets_tracks --load-data
